...
public class Game : RefSingleton<Game> {
    ...
    IEnumerator ResultProcess(RESULT result,int point)
    {
        ...
        Task.Run(async () =>
        {
            ...
            Debug.Log("sending " + Convert.ToString(this.Money) + " to backend");
            await CatervaNetworkManager.Instance.leaderboardControllerClient.UpdateEntryAsync("default",
                new UpdateEntryDto {Score = Convert.ToDouble(this.Money)}
                );
        });
